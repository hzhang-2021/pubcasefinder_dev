...
my $sth1 = $dbh->prepare("INSERT INTO casemini_OntoTermHPSynonymJa(OntoID,OntoSynonym,OntoVersion) VALUES (?,?,'')");
my $sth2 = $dbh->prepare("UPDATE casemini_OntoTermHPInformation SET OntoSynonymJa=? WHERE OntoID = ?");
...
sub _get_hpo_from_db {
	my %data = ();
	my $sth = $dbh->prepare("SELECT OntoID,OntoNameJa FROM casemini_OntoTermHPInformation where OntoID NOT IN (SELECT OntoDescendantID FROM casemini_OntoTermHPDescendant WHERE OntoID='HP:0000005') AND OntoID!='HP:0000118'");
	$sth->execute();
