...
sub _get_hpo_from_db {
	...
	my $sth = $dbh->prepare("SELECT A.OntoID,A.OntoName,A.OntoNameJa,A.OntoSynonym,A.OntoDefinition FROM casemini_OntoTermHPInformation as A, IndexFormHP as B where A.OntoID=B.uid AND A.OntoID NOT in (SELECT OntoDescendantID FROM casemini_OntoTermHPDescendant WHERE OntoID='HP:0000005') AND A.OntoID != 'HP:0000118'");
	$sth->execute();
