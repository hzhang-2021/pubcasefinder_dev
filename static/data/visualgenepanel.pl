...
our $URL_PANELS = "https://panelapp.genomicsengland.co.uk/api/v1/panels/";
...
my $sth1 = $dbh->prepare("INSERT INTO vgp(panel_id,panel_name) VALUES (?,?)");
my $sth2 = $dbh->prepare("INSERT INTO vgp_genes(panel_id,gene_id,gene_name) VALUES (?,?,?)");
...
foreach my $panel (@panel_list) {
	...
	$sth1->execute('PA:'. $panel_id, $panel_name);
    ...
    if ($response->is_success){
        ...
        for my $gene (@{$json->{'genes'}}){
			my $gene_symbol = $gene->{'entity_name'};
			if(exists($gene->{'gene_data'}{'hgnc_symbol'}) && $gene->{'gene_data'}{'hgnc_symbol'}){
				$gene_symbol = $gene->{'gene_data'}{'hgnc_symbol'};
			}

			if(exists($gene_hash{$gene_symbol})){
				my $gene_id = $gene_hash{$gene_symbol};
				$sth2->execute('PA:'. $panel_id, 'ENT:' .$gene_id, $gene_symbol);
			}else{
