...
;(function ($) {
	...
	const	LANGUAGE_JA='ja', LANGUAGE_EN='en',
			LANGUAGE = {
				[LANGUAGE_JA]:{ hintText: "Filter panels"	},
				[LANGUAGE_EN]:{	hintText: "Filter panels" }
			};

	const	UISETTING_TAG_SIZE_S = 's', UISETTING_TAG_SIZE_M = 'm', UISETTING_TAG_SIZE_L = 'l',
	...
	var DEFAULT_SETTINGS = {
		...
		hintText:		LANGUAGE[LANGUAGE_EN].hintText,
		noResultsText:	"No results",
		searchingText:	"Searching...",
		deleteText:		"&#215;",
		animateDropdown:true,
		placeholder:	LANGUAGE[LANGUAGE_EN].hintText,
		theme:			"facebook",
		zindex:			1999,
		resultsLimit:	null,
		enableHTML:		false,
		...
		onReady:			null,
		onClickDelete:		null,
		onShownScrollbar:	null,
		onHideScrollbar:	null,
		onChangeHPOnum:		null,
	...
	var methods = {
		...
		reset_hpo_list: function(hpo_id_list_str) {
			this.data("tokenInputObject").reset_hpo_list(hpo_id_list_str);
			return this;
		},
		get_hpo_list_str: function() {
			return this.data("tokenInputObject").get_hpo_list_str();
		},
		set_focus: function() {
			this.data("tokenInputObject").set_focus();
			return this;
		},
		...
		trigger_resize_input: function(){
			this.data("tokenInputObject").trigger_resize_input();
			return this;
		},
	...
	$.TokenList = function (input, url_or_data, settings) {
	  ...
	  const INPUT_BOX_ID = $(input).data("settings").idPrefix + input.id;
...
//						$(this).val("");
	  ...
	  var token_list = $("<ul />")
		  .attr("id", "UL-HPO-TOKEN-INPUT-LIST-" + input.id)
	  ...
	  let ul_id = "UL-HPO-TOKEN-INPUT-LIST-" + input.id;
	  const ulElement = document.getElementById(ul_id);
	  const wrapperElement = ulElement.parentElement;
	  const resizeObserver = new ResizeObserver(() => {
		  if (wrapperElement.scrollHeight > wrapperElement.clientHeight) {
	  		  var callback1 = $(input).data("settings").onShownScrollbar;
			  if(typeof callback1 === "function") callback1(); 
		  } else {
		  	  var callback1 = $(input).data("settings").onHideScrollbar;
			  if(typeof callback1 === "function") callback1();
		  }
  	  });
	  ...
	  this.reset_hpo_list = function(hpo_id_list_str){
		  ...
		  saved_tokens.length=0;
		  token_count = 0;
		  input_box.val('');
		  input_box.attr("placeholder", settings.placeholder);

		input_box.width(1);
		  if(hpo_id_list_str.length > 0){
			  let hpo_id_list = hpo_id_list_str.split(',');
			  for(let i=0; i<hpo_id_list.length; i++){
				let hpo_id = hpo_id_list[i];
				let hpo_name = hpo_cache[hpo_id];
				insert_token({id:hpo_id,name:hpo_name});
			  }
			  input_box.attr("placeholder", null);
		  }
		  resize_input();
	  ...
	  this.get_hpo_list_str = function(){
		return hiddenInput.val();
	  };

	  this.set_focus = function(){
		let element = document.getElementById(INPUT_BOX_ID);
	  ...
	  this.trigger_resize_input = function(){
			input_box.width(1);
		  resize_input();
	  };
      ...
      $(window).on('resize', function(e) {
		input_box.width(1);
		resize_input();
		console.log('resize_input triggered on windows resize');
      });
	  ...
	  function add_token (item) {
		  ...
		  var callback1 = $(input).data("settings").onChangeHPOnum;
		  if(typeof callback1 === "function") {
			callback1(token_count);
		  }
	  }
	  ...
	  function excludeCurrent(results) {
		...
		function do_search() {
			...
			if(query && query.length) {
				...
				if(len >= $(input).data("settings").maxChars  && typeof $(input).data("settings").onLongerQuery === "function"){
					input_box.val("");
					hide_dropdown();
