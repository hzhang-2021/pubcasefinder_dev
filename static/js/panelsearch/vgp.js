...
let infoTranslations = {
    en: [
        { 'What is PanelSearch?': 'PanelSearch is a publicly-available database that allows you to search and utilize Virtual Gene Panels associated with <span class="total-panel-num">9,998</span> human diseases. Virtual Gene Panels can be used for efficient clinical interpretation of genome sequences.' },
    ...
    ja: [
        { 'PanelSearchとは？': 'PanelSearchは、ヒトの<span class="total-panel-num">9,998</span>疾患に関連するVirtual Gene Panelを検索し、利活用できる公開データベースです。Virtual Gene Panelはゲノム配列の効率的な臨床的解釈に役立てることができます。' },
    ...
    ko: [
        { 'PanelSearch란?': 'PanelSearch는 인간의 <span class="total-panel-num">9,998</span> 질환과 관련된 Virtual Gene Panel을 검색하고 활용할 수 있는 공개 데이터베이스입니다. Virtual Gene Panel은 게놈 배열의 효율적인 임상적 해석에 도움이 될 수 있습니다.' },
    ...
    zh: [
        { '什么是面板搜索？': '面板搜索是一个公开的数据库, 允许你搜索和利用与<span class="total-panel-num">9,998</span>种人类疾病相关的虚拟基因面板。虚拟基因小组可用于对基因组序列的有效临床解释。' },
    ...
    zhcht: [
        { '什麼是面板搜索？': 'PanelSearch 是一個公開可用的數據庫，可讓您搜索和利用與 <span class="total-panel-num">9,998</span> 種人類疾病相關的虛擬基因面板。 Virtual Gene Panels 可用於對基因組序列進行高效的臨床解釋。' },
...
function vgp_set_total_panel_num(num){
	let total_num_str = num.toLocaleString("en-US");
	$(".total-panel-num").text(total_num_str);
}
