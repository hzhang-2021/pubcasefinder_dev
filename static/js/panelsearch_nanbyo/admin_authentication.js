/* ==================================
 * admin_authentication.js (Optimized)
 * ================================== */

const URL_LOAD_AUTHLOG = "/panelsearch_nanbyo_admin_load_authentication_log";
...
function getFilters(){
  ...
  return {
    filter_name:     utils_trimAllSpaces($('#filter_letter').val()),
...
function attachEvents(){
  ...
  utils_attach_auto_search("filter_letter", debounceSearch);
...
function debounceSearch(){
  ...
  const keyword = utils_trimAllSpaces($('#filter_letter').val());
  ...
  state.searchTimer = setTimeout(() => {
    submitWithLoading(
      ...
      getFilters(),
      logs => {
        if (utils_trimAllSpaces($('#filter_letter').val()) === keyword) {
          state.logs = logs || [];
          $("#total_num").text(state.logs.length);
          renderTable(state.logs);
        }
...
function createRow(log, $tbody){
  ...
  const keyword = utils_trimAllSpaces($('#filter_letter').val()).toLowerCase();
  const regex = keyword
