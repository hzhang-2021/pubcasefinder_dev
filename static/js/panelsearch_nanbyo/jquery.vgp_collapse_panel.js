;(function ($) {
	...
	const	TARGET_GENE_LIST      = "gene_list",
			...
			KEY_NANDO_URL         = 'nando_url',
			KEY_OMIM_ID           = 'omim_id',
			KEY_OMIM_URL          = 'omim_url',
			KEY_ORPHA_ID          = 'orphanet_id',
			KEY_ORPHA_URL         = 'orphanet_url',
			KEY_ICD_ID            = 'icd10_id',
			KEY_ICD_URL           = 'icd10_url',
			KEY_KEGG_ID           = 'kegg_id',
			KEY_KEGG_URL          = 'kegg_url',
			CLASS_NANDO_ID        = 'nando_id',
			LANGUAGE_EN           = 'en',
			LANGUAGE_JA           = 'ja',
			KEY_LANGUAGE          = 'language',
			LINK_KEY_HASH = [
				{id: KEY_NANDO_ID, url: KEY_NANDO_URL, label_prefix: '', cls: CLASS_NANDO_ID},
				{id: KEY_OMIM_ID,  url: KEY_OMIM_URL,  label_prefix: '', cls: ''},
				{id: KEY_ORPHA_ID, url: KEY_ORPHA_URL, label_prefix: '', cls: ''},
				{id: KEY_ICD_ID,   url: KEY_ICD_URL,   label_prefix: '', cls: ''},
				{id: KEY_KEGG_ID,  url: KEY_KEGG_URL,  label_prefix: 'KEGG:', cls: ''}
			],
	...
	var _isExistVal = function(key, hash){
	...
	function _contruct_popup_content(hpo_id,popup_data, lang){
		...
		if(utils_isEmpty(name_ja) && utils_isEmpty(name_en)){
	...
	function _construct_data_panel_gene(url_str){
		let str = "<script type=\"module\" src=\"https:\/\/togostanza.github.io\/metastanza\/pagination-table.js\" async><\/script>" +
				  ...
				  "<togostanza-pagination-table " +
			  		"data-url=\""+url_str+"\" " +
  					"data-type=\"json\" " +
					"custom-css-url=\"/static/css/panelsearch_nanbyo/pagination-table.css\" " +
					"width=\"\" " +
  					"fixed-columns=\"1\" " +
  					"padding=\"0px\" " +
	...
	var methods = {
		init: function(options) {
			...
			for(let i=0; i<TARGET_LIST.length; i++){
				...
				let $a = $('<a>').addClass('ctl').data(KEY_TARGET,target).data(KEY_COUNT,count).text(label)
			...
			for(let i=0; i<LINK_KEY_HASH.length; i++){
				...
				if(id_key === KEY_NANDO_ID && setting[id_key].startsWith("NANDO:300000")){
					continue;
				}

				let url_key = LINK_KEY_HASH[i].url;
				let cls   = LINK_KEY_HASH[i].cls;
				if(!utils_isEmpty(url_key) && !utils_isEmpty(setting[id_key]) && !utils_isEmpty(setting[url_key])){
					if(setting[url_key].indexOf('|')){
						let url_lst = setting[url_key].split('|');
						let id_lst  = setting[id_key].split('|');
						for(let n=0;n<url_lst.length;n++){
							let text = LINK_KEY_HASH[i].label_prefix + id_lst[n];
							let $a = $("<a>").text(text).addClass('vgp-link')
											.attr( 'href', url_lst[n]).attr('target', '_blank')
            			                    .appendTo($list_show_button_panel_upper);
							
							if(!utils_isEmpty(cls)){
								$a.addClass(cls);
							}
						}
					}else{
						let $a = $("<a>").text(setting[id_key]).addClass('vgp-link')
				...
				}else if(utils_isEmpty(url_key) && !utils_isEmpty(setting[id_key])){
					$("<span>").text(setting[id_key]).appendTo($list_show_button_panel_upper);
				}
			}
		...
		get_nando_id_obj: function(){
			let $container = this;
			let target = "a." + CLASS_NANDO_ID;
			let arr = $container.find(target);
			return arr[0];
		}
	};
