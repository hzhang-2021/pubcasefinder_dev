...
const SERVER_POST_URL = "/get_hpo_by_text";
...
function _do_search_disease(){
	...
	//let url = "/search_disease/phenotype:" + 
...
function _add_match_by_tokeninput(hpo_id_in,hpo_name){
	...
	let send_url_str = "/pcf_share?share=annotation&text="+encodeURIComponent(term_in_text)+"&phenotype="+ encodeURIComponent(hpo_id);
	$.ajax({
...
function _add_match(hpo_data) {
	...
	let send_url_str = "/pcf_share?share=annotation&text="+encodeURIComponent(term_in_text)+"&phenotype="+ encodeURIComponent(hpo_data.hpo_id);
	$.ajax({	
		url:	  send_url_str,  // 通信先のURL
		type:	 "GET",// 使用するHTTPメソッド(get/post)
		async:	true,	// 使用するHTTPメソッド(true/false)
