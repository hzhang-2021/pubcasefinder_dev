const URL_LOAD_EMAIL            = "/common_admin_load_email";
const URL_LOAD_EMAIL_DETAIL     = "/common_admin_load_email_detail";
const URL_RESEND_EMAIL          = "/common_admin_resend_email";
const URL_MODIFY_EMAIL_TEMPLATE = "/common_admin_modify_email_template";
...
function _init(email_templates, email_settings){
	...
	$('#template_auth_type').val(AUTH_TYPE);
	on_change_template();
    ...
    $('#btn_template_save').on('click', function() {
		let subject_str = trimAllSpaces($('#template_subject').val());
		let body_str = trimAllSpaces($('#template_body').val());
...
function showEmailDetail(id) {
	...
	.then(email => {
		...
		$('#modalTime').text(_format_timestamp(timestamp));
