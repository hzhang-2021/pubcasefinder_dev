;(function ($) {
	...
	const URL_GET_RANKING_BY_HPO_ID                 = '/pcf_get_ranking_by_hpo_id',
		  ...
		  URL_PCF_FILTER_GET_ORPHA_ID_BY_NANDO_ID           = '/sparqlist/api/pcf_filter_get_orpha_id_by_nando_id',
	...
	const URL_PARA_TARGET         = 'target',
          ...
          URL_PARA_FILTER_SIMPLE  = 'filter_simple',
		  URL_PARA_SIZE           = 'size',
		  URL_PARA_LANG           = 'lang',
	...
	const SETTING_KEY_TARGET         = 'PCF-TARGET',
          ...
          SETTING_KEY_FILTER_SIMPLE       = 'PCF-FILTER-2',
		  ...
		  SETTING_KEY_CLEAR_FILTER_SIMPLE = 'on_clear_filter2',
		  SETTING_KEY_LOAD_FILTER_SIMPLE_TEXT = 'load_filter2_text',
		  SETTING_KEY_GET_SCHEME     = 'get_scheme',
	...
	var DEFAULT_SETTINGS = {
		...
		'add_token':				  null,
		[SETTING_KEY_CLEAR_FILTER_SIMPLE]: null,
		[SETTING_KEY_LOAD_FILTER_SIMPLE_TEXT]: null,
		[SETTING_KEY_GET_SCHEME]:     null
	};
	...
	const LOGICAL_NOT = "LOGI-NOT",LOGICAL_AND = "LOGI-AND",LOGICAL_OR = "LOGI-OR",
		...
		FILTER_URL_HASH = {
			...
			[TARGET_ORPHANET]: {
				...
				[FILTER_TYPE_NANDO] : URL_PCF_FILTER_GET_ORPHA_ID_BY_NANDO_ID,
	...
	function _contruct_ranking_cache_key(setting) {
		...
		if((SETTING_KEY_FILTER     in setting && !_isEmpty(setting[SETTING_KEY_FILTER])) || 
           ...
           (SETTING_KEY_FILTER_SIMPLE   in setting && !_isEmpty(setting[SETTING_KEY_FILTER_SIMPLE]))){
	...
	function _get_ranking_data_without_filter_from_cache(setting){
		let setting_without_filter = $.extend(true,{}, setting,{[SETTING_KEY_FILTER]:'', [SETTING_KEY_FILTER_VGP]:'', [SETTING_KEY_FILTER_SIMPLE]:''});
	...
	function _construct_url(url_key, setting){
		...
		if(url_key === URL_GET_RANKING_BY_HPO_ID){
			...
			url_str = _construct_url_str(URL_GET_RANKING_BY_HPO_ID_WITH_FILTER,
                                         {[URL_PARA_TARGET]    : setting[SETTING_KEY_TARGET], 
                                          ...
                                          [URL_PARA_FILTER_SIMPLE]  : setting[SETTING_KEY_FILTER_SIMPLE]
                                         }
                      );
		...
		}else if(url_key === URL_PCF_FILTER_GET_ORPHA_ID_BY_NANDO_ID){
			url_str = _construct_url_str(URL_PCF_FILTER_GET_ORPHA_ID_BY_NANDO_ID,{[URL_PARA_NANDO_ID]: setting[SETTING_KEY_ID_LST].replaceAll(/NANDO:/g,"")});
    ...
    function _clear_filter_simple(){
        let target          = _get_active_target();
		let $panel          = tab_panel_lst[target];
		let current_setting = $panel.data(KEY_SETTING_OBJECT);
        if(_isFunction(current_setting[SETTING_KEY_CLEAR_FILTER_SIMPLE])){
            current_setting[SETTING_KEY_CLEAR_FILTER_SIMPLE]();
        }
	}
	...
	function _show_result(setting){
		...
		if(isFirstLoad){
			...
			let guide_scheme = "";
			if(_isFunction(setting[SETTING_KEY_GET_SCHEME])){
				guide_scheme = setting[SETTING_KEY_GET_SCHEME]();
			}
			let $guide_btn = $('<div>').attr('id', target + "_pcf_filter_guide_button").addClass(`pcf-filter-guide-button ${guide_scheme}`).text("Basic Filter").appendTo($top_panel);
			$(`<img src="/static/images/pcf/filter.svg"></img>`).prependTo($guide_btn);
...
"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"16\" height=\"16\" fill=\"currentColor\" class=\"bi bi-files\" viewBox=\"0 0 16 16\"><path d=\"M13 0H6a2 2 0 0 0-2 2 2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h7a2 2 0 0 0 2-2 2 2 0 0 0 2-2V2a2 2 0 0 0-2-2m0 13V4a2 2 0 0 0-2-2H5a1 1 0 0 1 1-1h7a1 1 0 0 1 1 1v10a1 1 0 0 1-1 1M3 4a1 1 0 0 1 1-1h7a1 1 0 0 1 1 1v10a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1z\"\/><\/svg> Copy selection"
			...
			if(target !== TARGET_CASE){
				let $filter_panel = $('<div>').attr('id', target + "_list-filter_simple").addClass("list-header-down").appendTo($header_wrapper);
				if(setting[SETTING_KEY_FILTER_SIMPLE]) $filter_panel.addClass('active');

				$('<span>').addClass("material-symbols-outlined").text('check').addClass('ml-3 mr-1').appendTo($filter_panel);
				let filter_text = '';
				if(setting[SETTING_KEY_FILTER_SIMPLE] && _isFunction(setting[SETTING_KEY_LOAD_FILTER_SIMPLE_TEXT])){
		            filter_text = setting[SETTING_KEY_LOAD_FILTER_SIMPLE_TEXT](setting[SETTING_KEY_FILTER_SIMPLE],setting[SETTING_KEY_LANG]);
			    }
				$('<span>').text('Filters applied: '+filter_text+'. ').addClass('mr-1').appendTo($filter_panel);
				$('<span>').addClass('clickable').text('Clear all')
					   .click(function(){
						  _clear_filter_simple();
					   })
	...
	function _run_pcf_filter_logical_case(ranking_data_without_filter,items_total_hash,result_ranking_id_hash,filter_vgp_gene_hash,target,setting,items_filter_simple_hash,result_filter_simple_ranking_id_hash){
		...
		if(_isEmpty(result_ranking_id_hash) && _isEmpty(result_filter_simple_ranking_id_hash)){
	...
	function _run_pcf_filter_logical(ranking_data_without_filter,items_total_hash,result_ranking_id_hash,filter_vgp_gene_hash,target,setting,items_filter_simple_hash,result_filter_simple_ranking_id_hash){
        ...
        let no_filter_simple = true;
        let filter_simple_list_str= setting[SETTING_KEY_FILTER_SIMPLE];
        if(filter_simple_list_str.length > 0) no_filter_simple = false;

		if(no_filter && no_filter_simple){
			...
			if(no_filter_vgp){
				...
				if(target === TARGET_CASE){
					_run_pcf_filter_logical_case(ranking_data_without_filter,items_total_hash,result_ranking_id_hash,filter_vgp_gene_hash,target,setting,items_filter_simple_hash,result_filter_simple_ranking_id_hash);
					return;
				}
			...
			let ranking_filter_simple_id_list = {};
            if(!no_filter_simple){
				ranking_data_without_filter.forEach(function(item){
					ranking_filter_simple_id_list[item.id] = 1;
				});

				Object.keys(items_filter_simple_hash).sort().forEach(function(i_str){

					let logical_type   = items_filter_simple_hash[i_str].logical_type;

					let hash = result_filter_simple_ranking_id_hash[i_str];
					...
					if(logical_type === LOGICAL_AND_NOT){
						Object.keys(ranking_filter_simple_id_list).forEach(function(ranking_id){
							if(ranking_id in filter_id_list){
								delete ranking_filter_simple_id_list[ranking_id];
							}
						});
					}else if(logical_type === LOGICAL_AND){
						Object.keys(ranking_filter_simple_id_list).forEach(function(ranking_id){
							if(!(ranking_id in filter_id_list)){
								delete ranking_filter_simple_id_list[ranking_id];
							}
						...
						Object.keys(filter_id_list).forEach(function(ranking_id){
							ranking_filter_simple_id_list[ranking_id] = 1;
						});
					}
				});
			}

			if(no_filter && no_filter_simple===false){
				ranking_id_list = ranking_filter_simple_id_list;
			}else if(no_filter===false && no_filter_simple===false){
				Object.keys(ranking_id_list).forEach(function(ranking_id){
					if(!(ranking_id in ranking_filter_simple_id_list)){
						delete ranking_id_list[ranking_id];
					}
				});
			}
	...
	function _run_pcf_search_ranking_data(setting){
		...
		// 4. filter_simple
		let type_filter_simple = 'filter_simple';
		let items_filter_simple_hash = {};
		let result_filter_simple_ranking_id_hash = {};
		let filter_simple_list_str = setting[SETTING_KEY_FILTER_SIMPLE];		
		if(filter_simple_list_str.length > 0){
			// analyze filter one by one
			let filter_simple_list = filter_simple_list_str.split(',');

            let idx_items_filter_simple_hash = 0;
			for(let i=0; i<filter_simple_list.length; i++){
				let str_filter_id = filter_simple_list[i];
				let logical_type  = _get_logical_by_filter_id(str_filter_id, i);
				let filter_type   = _get_filter_type_by_filter_id(str_filter_id);
				let id            = _get_id_from_filter_id(str_filter_id)
				let url_str       = FILTER_URL_HASH[target_str][filter_type];
				...
				// keep the item 
				items_filter_simple_hash[idx_items_filter_simple_hash] ={
					'index'        : idx_items_filter_simple_hash,
					'str_filter_id': str_filter_id,
					'id'           : id,
					'filter_type'  : filter_type,
					'logical_type' : logical_type
				...
				if(!_isEmpty(url_str)){
                    let url_str_full = _construct_url(url_str,{[SETTING_KEY_ID_LST]:id});
					let obj = {[SETTING_KEY_URLSTR]:url_str_full, 'type': type_filter_simple, 'index': idx_items_filter_simple_hash};
					ajax_item_list.push(obj);
				}else{
					// no need to get data through ajax
					result_filter_simple_ranking_id_hash[idx_items_filter_simple_hash] = {};
				}
                idx_items_filter_simple_hash++;
			}
		}
		...
		var callback_success = function(data,item){
			...
			}else if(item.type === type_filter_simple){
				let idx = item.index;
				result_filter_simple_ranking_id_hash[idx] = {};
				if(!_isEmpty(json_data)){
					if(target_str === TARGET_CASE){
						if(!_isEmpty(json_data) && _isArray(json_data)){
							json_data.forEach(function(target_data_id){
								result_filter_simple_ranking_id_hash[idx][target_data_id] = 1;
							});
						}
					}else{
						for(let jid in json_data){
		...
		var callback_after_all_call = function(){
			_run_pcf_filter_logical(ranking_data_without_filter,items_total_hash,result_ranking_id_hash,filter_vgp_gene_hash,target_str,setting,items_filter_simple_hash,result_filter_simple_ranking_id_hash);
		};
		...
		if(ajax_item_list.length === 0){
			if(Object.keys(result_ranking_id_hash).length >0 || Object.keys(filter_vgp_gene_hash).length >0 || Object.keys(result_filter_simple_ranking_id_hash).length > 0){
				// no need to do ajax
	...
	var methods = {
		init: function(options) {
			...
			if(_isExistVal(URL_PARA_TARGET,     options)) setting[SETTING_KEY_TARGET]         = options[URL_PARA_TARGET];
            ...
            if(_isExistVal(URL_PARA_FILTER_SIMPLE,   options)) setting[SETTING_KEY_FILTER_SIMPLE]       = _modify_filter_string(options[URL_PARA_FILTER_SIMPLE]);
			...
			if(_isExistVal(SETTING_KEY_CLEAR_FILTER_SIMPLE,options)) setting[SETTING_KEY_CLEAR_FILTER_SIMPLE]= options[SETTING_KEY_CLEAR_FILTER_SIMPLE];
			if(_isExistVal(SETTING_KEY_LOAD_FILTER_SIMPLE_TEXT,options)) setting[SETTING_KEY_LOAD_FILTER_SIMPLE_TEXT]= options[SETTING_KEY_LOAD_FILTER_SIMPLE_TEXT];
			...
			if(_isExistVal(SETTING_KEY_GET_SCHEME,options)) setting[SETTING_KEY_GET_SCHEME]= options[SETTING_KEY_GET_SCHEME];

			if(_isExistVal('add_token', options)) setting['add_token'] = options['add_token'];
			
			if(setting[SETTING_KEY_LANG] !== LANGUAGE_JA && setting[SETTING_KEY_FILTER_SIMPLE]){
				let lst = setting[SETTING_KEY_FILTER_SIMPLE].split(',');
				let filtered_lst = lst.filter(id =>!id.toLowerCase().includes('nando'));
				let str = filtered_lst.join(',');
				newStr = str.startsWith('AND_') ? str.slice(4) : str;
				setting[SETTING_KEY_FILTER_SIMPLE] = newStr;
			} 
...
*/
        ...
        update_filter_simple: function(filter_simple_str){
            let new_setting = {[SETTING_KEY_FILTER_SIMPLE]: filter_simple_str};
            _clear_all_and_update_setting(new_setting);
            let current_target = _get_active_target();
            _selectTab(current_target);
        },
		...
		download: function(hpo_id_list, mode, format){
			...
			if(mode === DOWNLOAD_MODE_ALL_SHOWN){
				...
				if(setting[SETTING_KEY_FILTER] || setting[SETTING_KEY_FILTER_VGP] || setting[SETTING_KEY_FILTER_SIMPLE]){
					//filtered
