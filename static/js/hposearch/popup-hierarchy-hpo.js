...
*/
		...
		function getOriginalTokenInputItemNodes(){
			...
			return $("#" + tokeninput_settings.tokeninput_ul_id).children('li.'+tokeninput_classes['token']).not('.'+current_settings.cssTokenClass).toArray();
		}
		...
		function _addTokenInputItem(token,selectedToken){
			...
			let str = current_settings.nodeName+'.'+current_settings.cssSelectedPhenotypeClass+ ' ul.'+tokeninput_classes['tokenList'].split(/\s+/).join('.')+'.'+current_settings.cssTokenListClass+'>li.'+tokeninput_classes['token'];
		...
		function executionAddOrReplace(e){
			...
			if(params.exec==='add'){
				if(current_settings.use_webgl || current_settings.is_hierarchy_fullscreen){
					...
					*/
					$("#"+input.id).tokenInput("add", new_token);
					//$("#tokeninput_hpo").tokenInput("add", new_token);
				}
			}
			else if(params.exec==='replace'){
				var $selectedToken = $("#" + tokeninput_settings.tokeninput_ul_id+'>li.'+tokeninput_classes['selectedToken']+'.'+current_settings.cssTokenClass);
				if(!current_settings.use_webgl && !current_settings.is_hierarchy_fullscreen){
					$selectedToken = $("#" + tokeninput_settings.tokeninput_ul_id+'>li.selected_at_popup');
				}
...
*/			
		...
		if(isObject(tokeninput_settings) && tokeninput_settings.classes) {
			if(isObject(tokeninput_classes) && isString(tokeninput_classes['tokenList']) && isString(tokeninput_classes['token'])){
				...
				tokeninput_selector = "#" + tokeninput_settings.tokeninput_ul_id+'>li.'+tokeninput_classes['token'];//+'>p';
