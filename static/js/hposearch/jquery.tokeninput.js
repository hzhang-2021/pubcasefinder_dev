...
;(function ($) {
    ...
    const   SERACH_TYPE_HPO='hpo',
        	SERACH_TYPE_DISEASE_ICD10='icd_10',
			SERACH_TYPE_DISEASE_OMIM_ORPHA='omim_orpha',
			SERACH_TYPE_DISEASE_NANBYO='nanbyo',
			ID_PREFIX={
				[SERACH_TYPE_HPO]:                "token-input-",
				[SERACH_TYPE_DISEASE_ICD10]:      "token-input-icd10-",
				[SERACH_TYPE_DISEASE_OMIM_ORPHA]: "token-input-omim_orpha-",
				[SERACH_TYPE_DISEASE_NANBYO]:     "token-input-nanbyo-"
	        },
			LANGUAGE_JA='ja',
			LANGUAGE_EN='en',
			LANGUAGE = {
				[LANGUAGE_JA]:{
					hintText:{
						[SERACH_TYPE_HPO]:                "患者の兆候または症状を入力",
						[SERACH_TYPE_DISEASE_ICD10]:      "疾患名を入力",
						[SERACH_TYPE_DISEASE_OMIM_ORPHA]: "疾患名を入力",
						[SERACH_TYPE_DISEASE_NANBYO]:     "疾患名を入力"
					} 
	            },
				[LANGUAGE_EN]:{
					hintText:{
						[SERACH_TYPE_HPO]:                "Type in patient's signs and symptoms",
						[SERACH_TYPE_DISEASE_ICD10]:      "Type in disease name",
						[SERACH_TYPE_DISEASE_OMIM_ORPHA]: "Type in disease name",
						[SERACH_TYPE_DISEASE_NANBYO]:     "Type in disease name"
					} 
				}
        	};
    ...
    var DEFAULT_SETTINGS = {
        ...
        hintText:        LANGUAGE[LANGUAGE_JA]['hintText'][SERACH_TYPE_HPO],
        noResultsText:   "No results",
        searchingText:   "Searching...",
        deleteText:      "&#215;",
        animateDropdown: true,
        placeholder:     LANGUAGE[LANGUAGE_JA]['hintText'][SERACH_TYPE_HPO],
        theme:           null,
        zindex:          1999,
        resultsLimit:    null,
        enableHTML:      false,

        resultsFormatter: function(item, search_type) {
            var id      = item['id'].replace(/_ja$/g,'');
            var name    = item['name'];
            var synonym = item['synonym'];
            var theme   = this.theme ? '-'+this.theme : '';

            var value   = '<li class=\"token-input-token-results'+theme+'\ d-flex flex-row">'+
                            '<div style=\"white-space:nowrap;\">';
            if((!search_type) || search_type === SERACH_TYPE_HPO){
                value  +=     '<span class=\"token-input-token-word'+theme+' token-input-token-information'+theme+' glyphicon glyphicon-info-sign\"></span>'
            }
            value = value +   '&nbsp;'+
                            '</div>'+
                            '<div style=\"white-space:nowrap;\" class=\"id\ '+search_type+'">'+
                              '<span class=\"token-input-token-word'+theme+' token-input-token-id'+theme+' mr-2\">' +
...
var value   = '<li class=\"token-input-token-results'+theme+'\">'+
			...
			if((!search_type) || search_type === SERACH_TYPE_HPO){
				value = value +	  '<span class=\"token-input-token-word'+theme+' token-input-token-information'+theme+' glyphicon glyphicon-info-sign\"></span>'
			}
			value = value +	      '&nbsp;'+
                                '</td>'+
...
*/
        ...
        tokenFormatter: function(item, uisetting_tag_size, search_type) {
            var id    = item['id'].replace(/_ja$/g,'');
            var name  = item['name'];
            var theme = this.theme ? '-'+this.theme : '';

			var observedClass = '';
			if(search_type === SERACH_TYPE_HPO){
				observedClass = item['is_observed']==='yes' ? 'observed' : 'notobserved';
			}
            
			let ret='<li class=\"token-input-li token-input-token-term'+theme+' '+ UISETTING_TAG_SIZE_CLASS[uisetting_tag_size] +'\">'+
                      '<p>'+
        ...
        idPrefix: ID_PREFIX[SERACH_TYPE_HPO],
		tokeninput_ul_id: null,
    ...
    var methods = {
        init: function(url_or_data_or_function, options) {
            ...
            settings.hintText         = LANGUAGE[LANGUAGE_JA]['hintText'][settings.search_type];
            settings.placeholder      = LANGUAGE[LANGUAGE_JA]['hintText'][settings.search_type];
            settings.idPrefix         = ID_PREFIX[settings.search_type];
			settings.tokeninput_ul_id = settings.idPrefix + '-' + this.id + "_ul_id";
    ...
    $.TokenList = function (input, url_or_data, settings) {
		...
		function _get_tokeninput_box_id(){
			return $(input).data("settings").idPrefix + input.id;
		}

		var input_box = $("<input type=\"text\" autocomplete=\"off\" autocapitalize=\"off\"/>")
			.css({outline: "none"})
			.attr("id", _get_tokeninput_box_id())
			.focus(function () {
		...
		var hiddenInput = $(input)
			.hide()
			.val("")
			.focus(function () {focusWithTimeout(input_box);})
			.blur(function () {
				input_box.blur();
		...
		var token_list = $("<ul />")
			.attr('id', $(input).data("settings").tokeninput_ul_id)
		...
		this.clear = function() {
			token_list.children("li").each(function() {
				if ($(this).children("input").length === 0) {
					// delete tags which are not input tag.
					delete_token($(this));
				}
			});
		};
		...
		this.add_hpo_list = function(hpo_list){
			...
			setTimeout(function() {
				let tokeninput_box_id = _get_tokeninput_box_id();
				let element = document.getElementById(tokeninput_box_id);
				element.scrollIntoViewIfNeeded();
				$('#'+tokeninput_box_id).focus();
			}, 100);
		};
		...
		function insert_token(item) {
			...
			var search_type = $(input).data("settings").search_type;
			var $this_token = $($(input).data("settings").tokenFormatter(item, uisetting_tag_size, search_type));
        ...
        function populateDropdown (query, results) {
            ...
            if(results && results.length) {
				...
				var search_type = $(input).data("settings").search_type;
                $.each(results, function(index, value) {
                    var this_li = $(input).data("settings").resultsFormatter(value, search_type);
                ...
                //}
                let node = dropdown;
                let $count_node = $('<div>').addClass('popup-hierarchy-hpo-number-of-hits');
                let node_content = '<b>' + 'ヒット件数 [__NUMBER__]&nbsp;&nbsp;'.replace('__NUMBER__', results_length) + '</b>';
                if(isSimilar){
                    node_content = '<b>' + 'ヒット件数 [__NUMBER__]&nbsp;&nbsp;'.replace('__NUMBER__', '0') + '</b>';
                    node_content = node_content +'<font color=\"#f8516d\">' + 'もしかして:' + '</font>';
                }
                ...
                if(node.get(0).firstElementChild){
                    ...
                    if(results_length===0) $firstElementChild.remove();
                }
                else{
        ...
        function run_similar_search(query) {
            ...
            $.ajax({
                ...
                if(data && $.isArray(data) && data.length > 0){
                    let results = [];
                    let search_type = $(input).data("settings").search_type;
                    for(let i=0;i<data.length;i++){

                        let obj = {synonym:   null, isSimilar: true}
                        obj.id      = data[i]['id'];
                        obj.name_en = data[i]['name_en'];
                        obj.name_ja = data[i]['name_ja'];
                        if(data[i]['synonym']){
                            obj.synonym = []
                            obj.synonym.push(data[i]['synonym']);
                        }

                        if(search_type === SERACH_TYPE_DISEASE_ICD10){
                            obj.name  = data[i]['name_ja'];
                            obj.group = data[i]['group'];
                            obj.disease_control_number = data[i]['disease_control_number'];
                        }else{

                            if(query.match(/[\u30a0-\u30ff\u3040-\u309f\u3005-\u3006\u30e0-\u9fcf]+/)){
                                    obj.name = (obj.name_ja) ? obj.name_ja : obj.name_en;
                            }else{
                                    obj.name = (obj.name_en) ? obj.name_en : obj.name_ja;
                            }

                            if(search_type === SERACH_TYPE_DISEASE_OMIM_ORPHA){
                                    obj.omim    = data[i]['omim'];
                                    obj.orphanet= data[i]['orphanet'];
                            }else if(search_type === SERACH_TYPE_DISEASE_NANBYO){
                                    obj.mondo   = data[i]['mondo'];;
                                    obj.notification_number = data[i]['notification_number'];;
                            }
                        }
                        results.push(obj);
                    }
