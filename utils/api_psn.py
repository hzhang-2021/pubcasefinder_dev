...
PANEL_VERSION_STATUS_DRAFT    = 'draft'
PANEL_VERSION_STATUS_RELEASED = 'released'
...
def _check_panel_validation(panel_id, dict_cursor):
    sql = "SELECT panel_id, is_valid FROM panelsearch_nando_panel WHERE panel_id = %s"
    dict_cursor.execute(sql, (panel_id,))
    result = dict_cursor.fetchone()

    if not result:
        return {'error': f"indicated panel({panel_id}) was not existed."}

    if result['is_valid'] == ENUM_VAL_NO:
        return {'error': f"indicated panel({panel_id}) was not valid due to ontology change."}
    else:
        return {'status': 'success'}


def _check_panel_validation_by_review(review_id, dict_cursor):
    sql = """
        SELECT p.panel_id, p.is_valid
        FROM panelsearch_nando_panel p
        INNER JOIN panelsearch_nando_entity_review r
            ON p.panel_id = r.panel_id
        WHERE r.review_id = %s
    """
    dict_cursor.execute(sql, (review_id,))
    result = dict_cursor.fetchone()

    if not result:
        return {'error': f"panel of indicated review({review_id}) was not existed."}

    if result['is_valid'] == ENUM_VAL_NO:
        return {'error': f"panel({result['panel_id']}) of indicated review({review_id}) was not valid due to ontology change."}

    return {'status': 'success'}



def api_psn_get_all_panel_info_hash(dict_cursor):
    return dict_cursor.execute("SELECT * FROM panelsearch_nando_panel")
...
def api_psn_get_panel_all_change_history(panel_id):
    ...
    with get_mysql_connection() as conn:
        try:
            with conn.cursor(MySQLdb.cursors.DictCursor) as cursor:
                ...
                cursor.execute(
                    "SELECT snapshot_json FROM panelsearch_nando_panel_snapshot WHERE panel_id = %s ",
                    (panel_id,)
                )
                ret_snapshot_json = cursor.fetchone() 
                response['snapshot_json'] = json.loads(ret_snapshot_json['snapshot_json'])
                ...
                sql = f"""
                    SELECT
                        ...
                        ua.entity_name,
                        ua.gene_id,
                        fe.entity_id AS old_entity_id,
                        ne.entity_id AS new_entity_id,
                        fe.rating_id AS old_rating_id,
                        ne.rating_id AS new_rating_id,
                        CASE
                            WHEN fe.rating_id IS NULL THEN 'ADD'
                            WHEN ne.rating_id IS NULL THEN 'DELETE'
                            ELSE 'CHANGE'
                        END AS rating_change
                    FROM panelsearch_nando_panel_change pc
                    JOIN panelsearch_nando_user_activity ua
                        ON ua.activity_id = pc.target_change_id
                    LEFT JOIN panelsearch_nando_entity fe
                        ON fe.entity_id = ua.former_entity_id
                    LEFT JOIN panelsearch_nando_entity ne
                        ON ne.entity_id = ua.entity_id
                    WHERE pc.panel_id = %s
                    AND pc.change_category = %s
                    AND (
                            fe.rating_id IS NULL
                            OR ne.rating_id IS NULL
                            OR fe.rating_id <> ne.rating_id
                        )
                """
                cursor.execute(sql,(panel_id,PANEL_CHANGE_CATEGORY_ENTITY))
                rows = cursor.fetchall()

                # create a dictionary to hold the changes for each panel_change_id
                changes_dict = {}
                for row in rows:
                    panel_change_id = row.pop('panel_change_id')
                    changes_dict[panel_change_id] = row

                response['changes'] = changes_dict
...
def api_psn_get_panel_version_info(panel_id, r_lang):
    ...
    SELECT DISTINCT
        ...
        pv.major_version, 
        pv.minor_version, 
        concat({pv_name_col}, '(', pv.major_version, '.', pv.minor_version,')') as panel_version, 
        pc.created_at, 
        pv.root_panel_id,
        pc.comment
    FROM panelsearch_nando_panel_change pc
...
def _get_newest_root_panel_version(root_panel_id,dic_cursor):
    ...
    dic_cursor.execute(
        """
        SELECT 
            panel_version_id, ontology_id, major_version, minor_version
        FROM 
...
def do_root_panel_minor_version_upgrade(root_panel_id,increment, comment, update_type, dic_cursor):
    ...
    dic_cursor.execute(
        ...
        INSERT INTO panelsearch_nando_panel_version
            (root_panel_id, ontology_id, major_version, minor_version, comment, update_type)
        VALUES (%s, %s, %s, %s, %s, %s)
        """,
        (
            root_panel_id, 
            former_version['ontology_id'], 
...
def record_panel_change(panel_id, change_category, target_change_id, user_id, comment, dic_cursor, subpanel_id=None):

    # record the change for this panel
    # the panel_id and the current panel_type are selected from panelsearch_nando_panel

    if change_category == PANEL_CHANGE_CATEGORY_SUBPANEL:
        if subpanel_id is None:
            raise ValueError("subpanel_id is required for SUBPANEL change.")

        dic_cursor.execute(
            """
            INSERT INTO panelsearch_nando_panel_change
            (panel_id, ontology_id, panel_type, change_category, target_change_id, changed_by, comment)
            SELECT
                p.panel_id,
                p.ontology_id,
                p.`type`,
            ...
            FROM panelsearch_nando_panel p
            LEFT JOIN panelsearch_nando_panel sp
                ON sp.panel_id = %s
            WHERE p.panel_id = %s
            """,
            (
        ...
        dic_cursor.execute(
            ...
            (panel_id, ontology_id, panel_type, change_category, target_change_id, changed_by, comment)
            SELECT 
                `panel_id`, `ontology_id`, `type`, %s, %s, %s, %s
            FROM panelsearch_nando_panel
            WHERE panel_id = %s
            """,
            (change_category, target_change_id, user_id, comment, panel_id)
        )
...
def _psn_panel_version_minor_up(
    ...
    if version_traces[0]['root_panel_id'] == panel_id:
        # this is the root panel change, just do minor version upgrade for this panel
    ...
    # this is the subpanel change, do minor version upgrade for all of the root panels
    for vt in version_traces:
        # this is the subpanel change, do minor version upgrade for all of the root panels
        do_root_panel_minor_version_upgrade(
            vt['root_panel_id'], 1, f"subpanel({panel_id}):{comment}", update_type, dic_cursor
        )
 
    #
    # record panel changes, this must be after the version upgrade, 
    # to make sure the change record is linked to the newest version
    #
...
def api_psn_regist_panel_entity_definition(user_id_change, data):
    ...
    with get_mysql_connection() as conn:
        try:
            with conn.cursor(MySQLdb.cursors.DictCursor) as cursor:
                ...
                insert_sql = f"""
                    INSERT INTO panelsearch_nando_entity (
                        panel_type, {col_str}
                    )
                    SELECT 
                        p.type, {placeholders}
                ...
                if cursor.rowcount != 1:
                    conn.rollback()
                    return {'error': f"panel_id {panel_id} not found"}
                ...
                # 4. for the new definition for panel entity 
                #    the panel version should do minor up
                if former_data is None:
                    change = 'added'
                elif new_data is None:
                    change = 'deleted'
                elif str(former_data['rating_id']) != str(new_data['rating_id']):
                    change = 'classified'
                else:
                    change = 'assessed'
                _psn_panel_version_minor_up(
                    ...
                    activity_id, 
                    f"entity({entity_name}) {change}",
                    cursor
                )
 
                # 5. Record panel version for this inserted entity
                # this must be called after the panel version minor up was finished.
...
def insert_entity_review(cursor, fields: dict):
    ...
    if "panel_type" in fields:
        # panel_type 已经提供，直接 INSERT
        cols = ", ".join(f"`{k}`" for k in fields.keys())
        vals = ", ".join(["%s"] * len(fields))
        ...
        sql = f"""
            INSERT INTO panelsearch_nando_entity_review (
                panel_type,
                {cols}
            )
            SELECT
        ...
        params.append(fields["panel_id"])

        cursor.execute(sql, params)

        if cursor.rowcount != 1:
            raise ValueError(f"panel_id {fields['panel_id']} not found")
...
def insert_review_comment(cursor, review_id, original_review_id, comment, created_at, user_id, user_id_change):

    cols = ['review_id', 'original_review_id', 'comment', 'user_id', 'user_id_change']
    parms = [review_id, original_review_id, comment, user_id, user_id_change]
...
def api_psn_regist_review(user_id_change, data):
    ...
    with get_mysql_connection() as conn:
        try:
            with conn.cursor(MySQLdb.cursors.DictCursor) as cursor:
                ...
                if comment:
                    new_review_comment_id = insert_review_comment(cursor, new_review_id, original_review_id, comment, None, user_id, user_id_change)
                ...
                activity_id = add_user_activity_log(cursor, user_id, user_id_change, former_data, new_data, USER_ACTIVITY_TARGET_REVIEW, action)
...
def build_review_sql(join_sql="", join_params=None, where_sql="", where_params=None, order_sql=""):
    ...
    SELECT DISTINCT
        ...
        concat(rpvpvp.panel_name_ja,'(',rpvpv.major_version, '.', rpvpv.minor_version,')') AS panel_version,
        ...
        u_change.user_type     as user_type_c,
        u_change.first_name_en as first_name_en_c,
        u_change.last_name_en  as last_name_en_c,
        u_change.affiliation   as affiliation_c,
        u_change.job_title     as job_title_c,
    ...
    INNER JOIN user_info_psn u_change
        ON r.user_id_change = u_change.id
    {join_sql} 
    """
...
def _fetch_entity_review_comments(where_sql, params):
    ...
    SELECT
        ...
        t1.panel_id,
        t1.panel_type,
        concat(p.panel_name_ja,'(', pv.major_version, '.',pv.minor_version, ')') as panel_version,
        t1.entity_type_id,
        t1.entity_name,
        t1.is_from_user,
        ...
        t4.user_id_change as comment_user_id_change,
        u_change.user_type     as user_type_c,
        u_change.first_name_en as first_name_en_c,
        u_change.last_name_en  as last_name_en_c,
        u_change.affiliation   as affiliation_c,
    ...
    INNER JOIN
        user_info_psn AS u
            ON t4.user_id = u.id
    INNER JOIN
        user_info_psn AS u_change
            ON t4.user_id_change = u_change.id
    WHERE
        {where_sql}
    ORDER BY
        t4.created_at DESC
...
def _copy_panel_entity_review(user_id_change, original_review_id, cur):
...
def api_psn_add_panel_entity_review_comment(user_id_change, review_id, original_review_id, new_comment):
    with get_mysql_connection() as conn:
        try:
            with conn.cursor(MySQLdb.cursors.DictCursor) as cur:
                ...
                review_comment_id = insert_review_comment(
                    cur, new_review['review_id'], original_review_id, new_comment, None, user_id_change, user_id_change
                )
...
def api_psn_modify_panel_entity_review_comment(user_id_change, review_id, original_review_id, review_comment_id, new_comment):
        ...
        try:
            with conn.cursor(MySQLdb.cursors.DictCursor) as cur:
                ...
                new_review_comment_id = insert_review_comment(
                    ...
                    original_review_id, 
                    new_comment, 
                    former_review_comment["created_at"],
                    former_review_comment['user_id'],
                    user_id_change
                ) 
...
def api_psn_delete_panel_entity_review_comment(user_id_change, review_id, original_review_id, review_comment_id):
    with get_mysql_connection() as conn:
        try:
            with conn.cursor(MySQLdb.cursors.DictCursor) as cur:
...
def add_user_activity_log(cursor, user_id, user_id_change, former_data, data, target, action):
    ...
    SELECT
        ...
        %(former_entity_id)s,
        %(former_review_id)s,
        %(former_review_comment_id)s,
        %(difference)s
    FROM panelsearch_nando_panel p
    WHERE p.panel_id = %(panel_id)s
    """
...
def get_select_activity_log_sql(join_clause, where_clause):
    ...
    SELECT
        ...
        concat(uapvp.panel_name_ja,'(',pv.major_version,'.',pv.minor_version,')') as panel_version,
        D.nando_id,
...
def api_psn_load_curator_check_history(curator_user_id):
    ...
    return { "suceed": 'done', "data":  id_list }
    ...
    with get_mysql_connection() as OBJ_MYSQL:
        with OBJ_MYSQL.cursor(MySQLdb.cursors.DictCursor) as cursor:
            ...
            SELECT DISTINCT
                ...
                concat(p.panel_name_ja,'(', pv.major_version, '.', pv.minor_version, ')') as name,
                pc.panel_type as panel_type,
...
def main():
    ...
    #print(api_psn_get_panel_all_change_history('NANDO:1200477'))
